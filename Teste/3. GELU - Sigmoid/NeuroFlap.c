...
double FUNÇÃO_ATIVAÇÃO_GELU (double x);
...
int main () {
    ...
    FILE* LOGS;
    LOGS = fopen ("LOG_GELU_Sigmoid.txt", "w");
...
double FUNÇÃO_ATIVAÇÃO_GELU (double x) {
    ...
    const double var1 = 0.044715;
    const double var2 = 0.797884;

    double y = x * x * x;

    
    return 0.5 * x * (1 + tanh(var2 * (x + var1 * y)));
...
double MULTILAYER_PERCEPTRON (double INPUT1, double INPUT2, double INPUT3, double INPUT4, PESSOA *x ) {
    ...
    int index = 0;
        ...
        for (int j = 0; j < NÚMERO_NEURÓNIOS_CAMADA_OCULTA; j++) {
            ...
            x -> OUTPUT_NEURÓNIO_HIDDEN_LAYER[j] = FUNÇÃO_ATIVAÇÃO_GELU (x -> NEURÓNIO_HIDDEN_LAYER[j]);
